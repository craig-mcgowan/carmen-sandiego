...
carmen=# SELECT * FROM country WHERE region = 'Southern Europe' ORDER BY population ASC LIMIT 1;
...
SELECT * FROM countrylanguage l WHERE countrycode = 'VAT';
-- carmen=# SELECT * FROM countrylanguage l WHERE countrycode = 'VAT';
...
SELECT * FROM country c JOIN countrylanguage l ON c.code = l.countrycode WHERE  
l.language = 'Italian' AND c.region = 'Southern Europe' AND l.percentage = 100;
...
SELECT * FROM city JOIN country c  ON c.code = city.countrycode WHERE  
c.name != city.name AND c.name = 'San Marino'; 
...
carmen=# SELECT * FROM city JOIN country c  ON c.code = city.countrycode WHERE  
c.continent = 'South America' AND city.name LIKE 'Serr%'; 
--  id  | name  | countrycode |     district     | population | code |  name  |   continent   |    region     | surfacearea  | indepyear | population | lifeexpectancy |    gnp    |  gnpold   | localname |  governmentform  |        headofstate        | capital | code2 
-- -----+-------+-------------+------------------+------------+------+--------+---------------+---------------+--------------+-----------+------------+----------------+-----------+-----------+-----------+------------------+---------------------------+---------+-------
--  265 | Serra | BRA         | Esp�rito Santo |     302666 | BRA  | Brazil | South America | South America | 8.547403e+06 |      1822 |  170115000 |           62.9 | 776739.00 | 804108.00 | Brasil    | Federal Republic | Fernando Henrique Cardoso |     211 | BR


-- Clue #6: We're close! Our South American agent says she just got a taxi at the airport, and is headed towards the capital! Look up the country's capital, and get there pronto! Send us the name of where you're headed and we'll follow right behind you!

SELECT * FROM city JOIN country c ON c.capital = city.id  WHERE c.name = 'Brazil';
--  id  |    name    | countrycode |     district     | population | code |  name  |   continent   |    region     | surfacearea  | indepyear | population | lifeexpectancy |    gnp    |  gnpold   | localname |  governmentform  |        headofstate        | capital | code2 
-- -----+------------+-------------+------------------+------------+------+--------+---------------+---------------+--------------+-----------+------------+----------------+-----------+-----------+-----------+------------------+---------------------------+---------+-------
